        ...
        <div class="row justify-content-center">
          <div class="col-12 col-lg-10 col-xl-10">
                ...
                <div class="row align-items-center">
                  <div class="col">
                    <ul class="nav nav-tabs nav-overflow header-tabs">
                      ...
                      <li class="nav-item">
                        <a class="nav-link active">Editar referencia</a>
                      </li>
                    </ul>
                  </div>
                </div>
            ...
            <ReferenciaForm
              ...
              submit-label="Actualizar referencia"
...
export default {
  ...
  methods: {
    ...
    syncClassificationMode() {
      const hasIndustria = !!this.form.industria_categoria;
      const hasProducto = !!this.form.producto_categoria;

      if (hasIndustria && hasProducto) this.classificationMode = 'ambas';
      else if (hasIndustria) this.classificationMode = 'industria';
      else if (hasProducto) this.classificationMode = 'producto';
      else this.classificationMode = 'industria';
    },
    load() {
      axios.get(this.$url + '/referencia/' + this.$route.params.id).then((response) => {
        const referencia = response.data;
        this.form = {
          _id: referencia._id,
          industria_categoria: referencia.industria_categoria ? referencia.industria_categoria._id : '',
          producto_categoria: referencia.producto_categoria ? referencia.producto_categoria._id : '',
          titulo: referencia.titulo || '',
          comuna: referencia.comuna || '',
          region: referencia.region || '',
          anio: referencia.anio ? String(referencia.anio) : '',
          marca: referencia.marca || '',
          modelo: referencia.modelo || '',
          potencia: referencia.potencia || '',
          caudal: referencia.caudal || '',
          presion: referencia.presion || '',
          descripcion: referencia.descripcion || '',
          estado: referencia.estado,
          imagen: referencia.imagen || ''
        };
        this.selectedComuna = this.form.comuna ? { value: this.form.comuna, text: this.form.comuna } : {};
        this.selectedDate = this.form.anio ? `${this.form.anio}-01-01` : '';
        this.syncClassificationMode();
      });
    },
    ...
    focusFirstError() {
      const order = ['clasificacion', 'industria_categoria', 'producto_categoria', 'titulo', 'comuna', 'anio', 'marca', 'modelo'];
    ...
    save() {
      ...
      axios.put(this.$url + '/referencia/' + this.form._id, data, {
        headers: hasFile
          ? { 'Content-Type': 'multipart/form-data', Authorization: this.$store.state.token }
          : { Authorization: this.$store.state.token }
      }).then(() => {
        this.$notify({ group: 'foo', title: 'EXITO', text: 'Referencia actualizada', type: 'success' });
        this.$router.push({ name: 'referencias' });
      }).catch((error) => {
        this.$notify({
          group: 'foo',
          title: 'ERROR',
          text: error.response && error.response.data && error.response.data.message ? error.response.data.message : 'No se pudo actualizar',
          type: 'error'
        });
      });
    }
  ...
  mounted() {
    Promise.all([this.fetchCategorias(), this.fetchMarcas()]).then(() => {
      this.load();
    });
  }
};
