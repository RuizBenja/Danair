        ...
        <div class="row justify-content-center">
          <div class="col-12 col-lg-10 col-xl-8">
                ...
                <div class="row align-items-center">
                  <div class="col">
                    <ul class="nav nav-tabs nav-overflow header-tabs">
                      <li class="nav-item">
                        <router-link to="/proyectos" class="nav-link">Todos los proyectos destacados</router-link>
                      </li>
                      <li class="nav-item">
                        <router-link to="/proyectos/create" class="nav-link">Nuevo proyecto destacado</router-link>
                      </li>
                      <li class="nav-item">
                        <a class="nav-link active">Editar proyecto destacado</a>
                      </li>
                    </ul>
                  </div>
                </div>
            ...
            <ProyectoForm
              ...
              submit-label="Actualizar proyecto destacado"
...
export default {
  name: 'EditProyectoApp',
  components: { Sidebar, TopNav, ProyectoForm },
  data() {
    return {
      form: {
        _id: '',
  ...
  methods: {
    load() {
      axios.get(this.$url + '/proyecto/' + this.$route.params.id).then((response) => {
        ...
        this.form = {
          ...
          seo_title: proyecto.seo_title || '',
          seo_description: proyecto.seo_description || '',
          estado: proyecto.estado,
          destacado: proyecto.destacado,
          imagen: proyecto.imagen || ''
        };
        this.showAdvanced = !!(proyecto.seo_title || proyecto.seo_description);
      });
    },
    onFile(event) {
      ...
      if (file.type !== 'image/webp') {
        ...
        if (this.$refs.proyectoForm.$refs.imagen) this.$refs.proyectoForm.$refs.imagen.value = '';
        return;
      }
    ...
    focusFirstError() {
      ...
      if (!firstKey) return;
      this.$nextTick(() => {
        const refsSource = this.$refs.proyectoForm && this.$refs.proyectoForm.$refs ? this.$refs.proyectoForm.$refs : {};
    ...
    save() {
      ...
      axios.put(this.$url + '/proyecto/' + this.form._id, data, {
        headers: hasFile
          ? { 'Content-Type': 'multipart/form-data', Authorization: this.$store.state.token }
          : { Authorization: this.$store.state.token }
      }).then(() => {
        this.$notify({ group: 'foo', title: 'EXITO', text: 'Proyecto actualizado', type: 'success' });
