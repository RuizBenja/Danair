                ...
                <div class="row align-items-center">
                  <div class="col">
                    <ul class="nav nav-tabs nav-overflow header-tabs">
                      ...
                      <li class="nav-item">
                        <a class="nav-link active">Nuevo proveedor</a>
                      </li>
                    </ul>
                  </div>
                </div>
            ...
            <div class="card">
              <div class="card-body">
                ...
                <div class="d-flex align-items-center gap-3">
                  <button class="btn btn-primary" @click="guardar">Crear proveedor</button>
...
export default {
  ...
  methods: {
    ...
    guardar() {
      ...
      axios.post(this.$url + '/registro_proveedor_admin', this.form, {
        headers: { 'Content-Type': 'application/json', Authorization: this.$store.state.token }
      }).then(() => {
        this.$notify({ group: 'foo', title: 'EXITO', text: 'Proveedor creado', type: 'success' });
        this.$router.push({ name: 'proveedor-index' });
      }).catch((error) => {
        const message = error.response?.data?.message || 'No se pudo crear';
