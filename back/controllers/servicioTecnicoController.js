...
const SERVICIOS_TECNICOS_DIR = path.join(UPLOADS_DIR, 'serviciotecnico');
ensureDir(UPLOADS_DIR);
ensureDir(SERVICIOS_TECNICOS_DIR);

function sanitizeServicioTecnicoData(data) {
  return {
...
function validateServicioTecnicoData(data, requireImage = false) {
...
const normalizeOrders = async () => {
  const items = await ServicioTecnico.find().sort({ orden: 1, createdAt: 1 });
  let changed = false;
...
const create = async (req, res) => {
  ...
  try {
    const data = sanitizeServicioTecnicoData(req.body);
    const last = await ServicioTecnico.findOne().sort({ orden: -1 });
    ...
    const validationError = validateServicioTecnicoData(data, true);
    if (validationError) return res.status(400).send({ message: validationError });

    const doc = await ServicioTecnico.create(data);
    return res.status(200).send(doc);
  } catch (e) {
    return res.status(500).send({ message: 'Error al crear servicio tecnico' });
  }
};
...
const update = async (req, res) => {
  ...
  const data = sanitizeServicioTecnicoData({ ...doc.toObject(), ...req.body });
  ...
  if (req.files && req.files.imagen) {
    ...
    if (doc.imagen) deleteFileIfExists(path.join(SERVICIOS_TECNICOS_DIR, doc.imagen));
    data.imagen = path.basename(file.path);
  } else {
    data.imagen = doc.imagen || '';
  }

  const validationError = validateServicioTecnicoData(data, false);
  if (validationError) return res.status(400).send({ message: validationError });
...
const reorder = async (req, res) => {
  ...
  const current = await ServicioTecnico.findById(id);
  ...
  const swap = await ServicioTecnico.findOne({ orden: targetOrder });
...
const obtener_imagen = async function(req,res){
  ...
  const imgPath = path.join(SERVICIOS_TECNICOS_DIR, img);
